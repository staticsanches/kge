...
class KotlinGameEngine(val appName: String) :
	...
	private fun Configurator.createWindow(): Window {
		...
		val windowHandle = GLFW.glfwCreateWindow(
			windowWidth, windowHeight, appName,
			if (fullScreen) GLFW.glfwGetPrimaryMonitor() else MemoryUtil.NULL, MemoryUtil.NULL
		)
		...
		if (!fullScreen) {
			if (centerWindow) {
				val mode = checkNotNull(GLFW.glfwGetVideoMode(GLFW.glfwGetPrimaryMonitor())) {
	...
	private fun Window.registerCallbacks() {
		...
		GLFW.glfwSetKeyCallback(windowHandle) { window, key, scancode, action, mods ->
			...
			val keyboardKeyAction = when (action) {
