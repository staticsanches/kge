...
interface CallbacksAddon {
	 ...
	 * Handle [GLFW] errors.
	 */
	fun onGLFWError(error: Int, message: Long): Unit =
		throw IllegalStateException("[GLFW] Error ${error.glfwErrorCode}: ${message.glfwErrorDescription}")

	/**
	 * Handle window resize.
	 */
	context(Window)
	fun onFrameBufferResize(width: Int, height: Int) = Unit

	/**
	 * Handle keyboard events.
	 */
	context(Window)
	fun onKeyEvent(key: KeyboardKey, newAction: KeyboardKeyAction, scancode: Int, newModifiers: KeyboardModifiers) =
		Unit
	...
	companion object {
		...
		private val errorCodes = APIUtil.apiClassTokens(
			{ _: Field?, value: Int -> value in 0x10001..0x1ffff }, null, GLFW::class.java
		).toMap()
