...
interface Vector2D<T> {
    ...
    fun clamp(
        v1: Vector2D<T>,
        v2: Vector2D<T>,
    ): Vector2D<T> = (this max v1) min v2

    fun lerp(
        v1: Vector2D<*>,
        t: Double,
    ): Double2D = this.toDouble2D() * (1.0 - t) + v1.toDouble2D() * t
    ...
    fun copy(
        x: T = this.x,
        y: T = this.y,
    ): Vector2D<T> = if (this.x == x && this.y == y) this else newInstance(x, y)
...
interface MutableVector2D<T> : Vector2D<T> {
    ...
    override infix fun max(other: Vector2D<T>): MutableVector2D<T> =
        copy(
            maxOf(x, other.x, this::compare),
            maxOf(y, other.y, this::compare),
        )

    override infix fun min(other: Vector2D<T>): MutableVector2D<T> =
        copy(
            minOf(x, other.x, this::compare),
            minOf(y, other.y, this::compare),
        )
    ...
    override fun clamp(
        v1: Vector2D<T>,
        v2: Vector2D<T>,
    ): MutableVector2D<T> = (this max v1) min v2

    override fun lerp(
        v1: Vector2D<*>,
        t: Double,
    ): MutableDouble2D = this.toDouble2D() * (1.0 - t) + v1.toDouble2D() * t
    ...
    override fun copy(
        x: T,
        y: T,
    ): MutableVector2D<T> = newInstance(x, y)
