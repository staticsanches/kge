...
class Decal private constructor(val sprite: Sprite, private val texture: IdentifiedResource<Int>) :
	...
	companion object {
		...
		context(Window)
		operator fun invoke(sprite: Sprite, filtered: Boolean = false, clamp: Boolean = true): Decal =
			Decal(
				sprite,
				IdentifiedResource("Decal", { Renderer.createTexture(filtered, clamp) }, { Renderer.deleteTexture(it) })
			)
