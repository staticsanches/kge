...
class Sprite(
	...
	fun duplicate(offset: Int2D, size: Int2D): Sprite =
		create(size.x, size.y, sampleMode) { x, y -> this[x + offset.x, y + offset.y] }
	...
	companion object {
		...
		fun load(fileName: String, sampleMode: SampleMode = SampleMode.NORMAL): Sprite =
			Sprite(PixelBufferService.load(fileName), sampleMode)

		fun load(url: URL, sampleMode: SampleMode = SampleMode.NORMAL): Sprite =
			Sprite(PixelBufferService.load(url), sampleMode)

		fun load(isProvider: () -> InputStream, sampleMode: SampleMode = SampleMode.NORMAL): Sprite =
			Sprite(PixelBufferService.load(isProvider), sampleMode)
