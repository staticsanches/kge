...
/**
 * The default decoder for an engine buffer of encoded image bytes. The payload
 * format is auto-detected by the platform backend (STB on the JVM,
 * `createImageBitmap` on the web).
 *
 * [data] is caller-owned and never closed here; the backend allocates the RGBA
 * buffer handed to `consume`, transferring ownership to the consumer.
 */
 ...
 * until [consume] transfers it to the consumer.
 */
